...
def create_user():
    ...
    c.execute("INSERT INTO users (first_name, last_name, dob, phone, address, username, password) VALUES (?, ?, ?, ?, ?, ?, ?)",
              (first_name, last_name, dob, phone, address, username, password))
    conn.commit()
...
# Frames
frame_personal = tk.LabelFrame(root, text="Personal Information", padx=12, pady=12)
...
# Date of Birth
label_dob = tk.Label(frame_personal, text="Date of Birth (YYYY-MM-DD)")
label_dob.grid(row=2, column=0, sticky=tk.W)
entry_dob = tk.Entry(frame_personal)
entry_dob.grid(row=2, column=1)

# Phone Number
label_phone = tk.Label(frame_personal, text="Phone Number")
...
def close_account():
    ...
    c.execute("DELETE FROM users WHERE account_number = %s", (account_number,))
    c.execute("DELETE FROM accounts WHERE account_number = %s", (account_number,))
    c.execute("DELETE FROM transactions WHERE account_number = %s", (account_number,))
    conn.commit()

    print("Account " + account_number + " has been closed")
...
def check_account_balance(id: int) -> int:
    sql = f"""SELECT balance FROM `{username}`.`user` where id = {id};"""
    cur = conn.cursor()
    cur.execute(sql)
    result = cur.fetchone()
    return result[0]
